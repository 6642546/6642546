...
class Page {
    ...
    public function show() {
        ...
        if ($upRow>0){
			...
			$this->prePg = "<a href='".$url."&".$p."=$upRow' ><i class='fa fa-angle-left'></i></a>";
        }else{
            $upPage="";
			$this->prePg = "<a><i class='fa fa-angle-left'></i></a>";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$url."&".$p."=$downRow'>".$this->config['next']."</a>";
			$this->nextPg = "<a href='".$url."&".$p."=$downRow' ><i class='fa fa-angle-right'></i></a>";
        }else{
            $downPage="";
			$this->nextPg = "<a><i class='fa fa-angle-right'></i></a>";
        }
        // << < > >>
        if($nowCoolPage == 1){
            $theFirst = "";

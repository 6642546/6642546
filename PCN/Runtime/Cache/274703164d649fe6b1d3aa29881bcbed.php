		...
		<!--hr style='margin-top:-3px;'-->
			  <div class="container">
			    <div class="row" >
			     ...
			     <form  method='post' style='margin-top:0;' enctype="multipart/form-data" class="needs-validation" novalidate id='approvalform'>
                 ...
                 <?php echo ($timeline); ?>
				   <div class="form-group row"  style='border-top:1px solid black;' >
							...
							<div class='col-sm-1'>
                                 ...
                                 <button type="button" class="btn btn-info" onclick="fomsubmit(1)" style='margin-top:10px;'>Close</button>
							</div>
				...
				for(var i = 1; i<rows.length; i++ ){
				  // for(var j = 0; j<rows[i].cells.length-1; j++ ){    // 遍历该行的 td
					...
					//excestatus+= $('#addPnList tbody tr:eq('+rows[i]+')').find('td:eq(3)').find('input').val()+';';
					 excestatus+=rows[i].cells[3].getElementsByTagName('input')[0].checked+';';
					 execuser+=rows[i].cells[4].innerHTML+';';
					 execdate+=rows[i].cells[2].innerHTML+';';
				  // }
				}
		...
		/* modal end*/
		 ...
		 $("#input-res-3").fileinput({
			   ...
			   // deleteUrl: '__URL_/del_file',
				...
				uploadAsync: false,
				//initialPreviewDownloadUrl:'http://10.120.1.243/webtools/PCN/Uploads/{filename}',//decodeURI('http://localhost/PCN/Uploads/{filename}'), // includes the dynamic `filename` tag to be replaced for each config

			    ...
			    <div class="row" >
			     ...
			     <form action='__URL__/update' method='post' style='margin-top:0;' enctype="multipart/form-data" class="needs-validation" novalidate>
				      <div class="form-group row"  style='border-top:1px solid black; font-size:20px;'>
						   ...
						   <input type="text" class="form-control col-form-label-sm col-sm-10" id="user_name" name='user_name' value='<?php echo ($vo["user_name"]); ?>' placeholder="<?php echo (L("user_name")); ?>" style='margin-top:10px;' required>	
						    <input type="text" class="form-control col-form-label-sm col-sm-10" id="id" name='id' value='<?php echo ($vo["id"]); ?>' placeholder="<?php echo (L("id")); ?>" style='margin-top:10px;' hidden>	
					  </div>
					   <div class="form-group row"  >
						   <label for="user_pwd" class="col-sm-2 col-form-label-sm"><?php echo (L("user_pwd")); ?> :</label>
						   <input type="password" class="form-control col-form-label-sm col-sm-10" id="user_pwd" name='user_pwd' value='<?php echo ($vo["user_pwd"]); ?>' placeholder="<?php echo (L("user_pwd")); ?>" style='margin-top:10px;'>					
					  </div>
					   <div class="form-group row"  >
						   <label for="role" class="col-sm-2 col-form-label-sm"><?php echo (L("role")); ?> :</label>
						    <select  class="form-control col-sm-10" id="role" name='role' value='<?php echo ($vo["role"]); ?>'>
								  <option></option>
								  <option  <?php if(($vo["role"]) == "admin"): ?>selected<?php endif; ?> value="admin" >admin</option>
								  <option <?php if(($vo["role"]) == "general"): ?>selected<?php endif; ?>  value='general'>general</option>
								  <option<?php if(($vo["role"]) == "guest"): ?>selected<?php endif; ?> value='guest'>guest</option>
							</select>				
					  </div>  
					   <div class="form-group row"  >
						   ...
						   <select  class="form-control col-sm-10" id="status" name='status'>
								  <option <?php if(($vo["status"]) == "1"): ?>selected<?php endif; ?> value="Valid"> Valid</option>
								  <option <?php if(($vo["status"]) == "0"): ?>selected<?php endif; ?> value="Invalid">Invalid</option>
							</select>						
					  </div>    
					  <div class="form-group row"  >
...
<script>
  /*Check Valide*/
    ...
    /*Check Valide end*/
	var rule='<?php echo ($vo["access_rule"]); ?>';
	var ra=rule.split(';');
	ra.forEach(function(item,index,array){
	  
	   $(".form-check-input[value='"+item+"']").prop("checked", true);
	})

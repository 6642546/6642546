		...
		<!--hr style='margin-top:-3px;'-->
			  <div class="container">
			    <div class="row" >
			     ...
			     <form  action='__URL__/update' method='post' style='margin-top:0;' enctype="multipart/form-data" class="needs-validation" novalidate id='exec_pe'>
					  ...
					  <div class="form-group row">
						...
						<input type="text"  class="form-control col-form-label-sm" id="exec_status" name='exec_status' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["exec_status"]); ?>"  readonly hidden >	
						<input type="text"  class="form-control col-form-label-sm" id="exec_date" name='exec_date' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["exec_date"]); ?>"  readonly hidden>	
						<input type="text"  class="form-control col-form-label-sm" id="exec_user" name='exec_user' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["exec_user"]); ?>"  readonly hidden>	
						<input type="text"  class="form-control col-form-label-sm" id="exec_remark" name='exec_remark' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["exec_remark"]); ?>"  readonly hidden>	
						<input type="text"  class="form-control col-form-label-sm" id="n_internal_pn" name='n_internal_pn' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["n_internal_pn"]); ?>"  readonly hidden >	
						<input type="text"  class="form-control col-form-label-sm" id="ptl_no" name='ptl_no' placeholder="<?php echo (L("cust_pn")); ?>" value="<?php echo ($vo["ptl_no"]); ?>"  readonly hidden>	
					  </div>
					   <div class="form-group row">
				   ...
				   <div class="form-group row"  style='border-top:1px solid black;' >
							...
							<div class='col-sm-2'  >
							     <!--button type="button" class="btn btn-info" onclick="fomsubmit(1)" style='margin-top:10px;'>Approval</button-->
								  <button type="button" class="btn btn-info" onclick="fomsubmit(1)" style='margin-top:10px;'>Exec</button>
							</div>
							<div class='col-sm-1'>
                                <button type="button" class="btn btn-outline-secondary" onclick="fomsubmit(2)" style='margin-top:10px;'>Close</button>
							</div>
					  ...
					  <div class="modal-dialog" role="document" >
						<div class="modal-content" style='width:1200px;margin-left:-300px;'>
						  ...
						  <div class="modal-header">
							<h4 class="modal-title" id="myModalLabel">Part Number List</h4>
						...
						<!-- Editable table -->
								...
								<div class="table-wrapper" style='width:1150px; margin-left:0;'>
...
<script>
    ...
    /*Check Valide end*/
		   ...
		   showModal=function(){
		    ...
		    modal_pn=function(){
				...
				for(var i = 1; i<rows.length; i++ ){
				  ...
				  if(rows[i].cells[4].innerHTML.indexOf("<input type")!=-1){
					 alert("第"+(i)+"行 包含Html标签"+rows[i].cells[4].innerHTML+'请按添加按钮消除!');
					 return;					
					}
					 joblist+=rows[i].cells[0].innerHTML+';';
					 cspnlist+=rows[i].cells[1].innerHTML+';';
					 csname+=rows[i].cells[2].innerHTML+';';
					 ninternal+=rows[i].cells[3].innerHTML+';';
					 ptl_no+=rows[i].cells[4].innerHTML+';';
					// excestatus+=$('#addPnList tbody tr:eq('+rows[i]+') td:eq(4) input').val()+';';
					//excestatus+= $('#addPnList tbody tr:eq('+rows[i]+')').find('td:eq(3)').find('input').val()+';';
					if(rows[i].cells[5].getElementsByTagName('input')[0].checked==true&&rows[i].cells[3].innerHTML==''){
					 alert('Internal Part Number Is Empty!');
                    // alert(rows[i].cells[3].getElementsByTagName('input')[0].checked);
					 return;
					
					}
					 excestatus+=rows[i].cells[5].getElementsByTagName('input')[0].checked+';';
					 execuser+=rows[i].cells[6].innerHTML+';';
					 execdate+=rows[i].cells[7].innerHTML+';';
					 execremark+=rows[i].cells[8].innerHTML+';';
				  // }
				}
				...
				$("#exec_status").val(excestatus.substr(0, excestatus.length - 1));
			    $("#exec_date").val(execdate.substr(0, execdate.length - 1));
				$("#exec_user").val(execuser.substr(0, execuser.length - 1));
				$("#exec_remark").val(execremark.substr(0, execremark.length - 1));
				$("#n_internal_pn").val(ninternal.substr(0, ninternal.length - 1));
				$("#ptl_no").val(ptl_no.substr(0, ptl_no.length - 1));
		...
		fomsubmit=function(stas){
			if (stas==1){
		        var form = document.getElementById('exec_pe');
				form.submit();
			}else{
		...
		/* modal tablestart*/
					...
					$(document).on("click", ".edit", function(){
						...
						$(this).parents("tr").find("td:not(:last-child)").each(function(){
							...
							if (i==7)
							{
								$(this).html('<input type="text" id="execUser"  class="form-control" value="' + $(this).text() + '">');
							}
							if ( i==4)
							{
								$(this).html('<input type="text" id="nInternal"  class="form-control" value="' + $(this).text() + '">');
							}
							if ( i==5)
							{
								$(this).html('<input type="text" id="ptlno" class="form-control" value="' + $(this).text() + '">');
							}
							if ( i==9)
							{
							...
							{					  
                               var time1 = new Date().Format("yyyy-MM-dd hh:mm:ss"); 
								$(this).html('<input type="text" class="form-control" value="' + time1 + '">');
							}
						});		
						$(this).parents("tr").find(".add, .edit").toggle();
						$(".add-new").attr("disabled", "disabled");
						$("#execUser").on("focus", function(){
								$(this).autocomplete({
									source: function (request, response)
									{
										$.ajax({
											url: "__URL__/getExecUser", //.asp, .aspx, .ashx 或.php都可以
											dataType: "json",
											data: {
							...
							$("#nInternal").on("focus", function(){
								$(this).autocomplete({
									source: function (request, response)
									{
										$.ajax({
											url: "__URL__/getPeMangementInfo", //.asp, .aspx, .ashx 或.php都可以
											dataType: "json",
											data: {
												key: request.term      //回傳的參數
											},

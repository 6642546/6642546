		...
		<!--hr style='margin-top:-3px;'-->
			  <div class="container">
			    <div class="row" >
			     ...
			     <form action='__URL__/update' method='post' style='margin-top:0;' enctype="multipart/form-data" class="needs-validation" novalidate>
					  ...
					  <div class="form-group row">
					    ...
					    <input type="text" class="form-control col-form-label-sm" id="internal_pn" name='internal_pn' placeholder="<?php echo (L("internal_pn")); ?>" style='width:85%;float:left' value="<?php echo ($vo["internal_pn"]); ?>" readonly required> 
						 ...
						 <input type="text" id="prem"  value="<?php echo ($vo["pre_m"]); ?>" hidden> 
                    ...
                    <div class="form-group row" style='border-top:1px solid black;border-bottom:1px solid balack;'>
							...
							<div class='col-sm-10' style='padding-top:20px;'>
									 <div class="form-group row">
										...
										<div class="col-sm-10">
										  <textarea class="form-control" id="before_change" placeholder="<?php echo (L("before_change")); ?>"  name='before_change' rows="6" required><?php echo ($vo["before_change"]); ?></textarea>
										   <div class="invalid-feedback">
										...
										<div class="col-sm-10" style='padding-top:10px;'>
										  <textarea class="form-control" id="after_change" placeholder="<?php echo (L("after_change")); ?>"  name='after_change' rows="6"  required><?php echo ($vo["after_change"]); ?></textarea>
										   <div class="invalid-feedback">
	                ...
	                <div class="form-group row"  style='border-top:1px solid black;' >
							...
							<div class='col-sm-10'>
									 <div class="form-group row"  style='padding-top:20px;'>
										...
										<div class="col-sm-10">
										  <textarea class="form-control" id="reason_change" placeholder="<?php echo (L("reason_change")); ?>"  name='reason_change' rows="6"  required><?php echo ($vo["reason_change"]); ?></textarea>
										   <div class="invalid-feedback">
                     ...
                     <div class="form-group row" style='border-top:1px solid black;border-left:1px solid balack;'>
							...
							<div class='col-sm-10'>
							    <?php if(($vo["affect_shipping_schedule"]) == "1"): ?><div class="form-check form-check-inline" style='padding-top:20px;'>
										  <input class="form-check-input" type="radio" name="affect_shipping_schedule" id="affect_shipping_schedule_yes" value="1" checked=true>
										  <label class="form-check-label" for="affect_shipping_schedule_yes"><?php echo (L("affect_shipping_schedule_yes")); ?></label>
										</div>
										<div class="form-check form-check-inline">
										  <input class="form-check-input" type="radio" name="affect_shipping_schedule" id="affect_shipping_schedule_no" value="0" >
                               ...
                               <?php else: ?>
									...
									<label for="affect_remark" class="col-form-label-sm"><?php echo (L("affect_remark")); ?> :</label>
					           	   	<input type="text" class="form-control col-form-label-sm" id="affect_remark" name='affect_remark' placeholder="<?php echo (L("affect_remark")); ?>" value="<?php echo ($vo["affect_remark"]); ?>">
					   ...
					   <div class="form-group row" style='border-top:1px solid black;border-left:1px solid balack;'>
							<div class='col-sm-2'  >
							  <p style='padding-top:20px;'> <?php echo (L("pre_manger_approve")); ?></p>
							</div>
							<div class='col-sm-10' style='padding-top:20px;'>
									<select name="pre_m" id="pre_m" class="form-control col-form-label-sm" required>
										<!--option <?php if(($vo["pre_m"]) == ""): ?>selected<?php endif; ?> value=""><?php echo (L("please_choose")); ?></option>
										<volist id="vo1" name="mlist">
										  <option value="<?php echo ($vo1['user_name']); ?>"><?php echo ($vo1['user_name']); ?></option>
										</volist-->
									  <option  value=""><?php echo (L("please_choose")); ?></option>
										<?php if(is_array($mlist)): foreach($mlist as $key=>$prem): if($vo["pre_m"] == $prem['user_name']): ?><option value="<?php echo ($prem['user_name']); ?>" selected ><?php echo ($prem['user_name']); ?></option>  
                                             <?php else: ?>
                                                 <option value="<?php echo ($prem['user_name']); ?>"><?php echo ($prem['user_name']); ?></option><?php endif; endforeach; endif; ?>	
									</select>
							</div>
					   ...
					   <div class="form-group row" style='border-top:1px solid black;border-left:1px solid balack;'>
							<div class='col-sm-2'  >
							  <p style='padding-top:20px;'> <?php echo (L("pcn_type")); ?></p>
							</div>
							<div class='col-sm-10' style='padding-top:20px;'>
									<select name="category" id="category" class="form-control col-form-label-sm"  required>										
									   <option  value=""><?php echo (L("please_choose")); ?></option>
										<?php if(is_array($pcntype)): foreach($pcntype as $key=>$pcn_category): if($vo["category"] == $pcn_category): ?><option value="<?php echo ($pcn_category); ?>" selected ><?php echo ($pcn_category); ?></option>  
                                             <?php else: ?>
                                                 <option value="<?php echo ($pcn_category); ?>"><?php echo ($pcn_category); ?></option><?php endif; endforeach; endif; ?>										
									</select>							
							</div>						
					 </div>
		...
		/* modal end*/
		 ...
		 $("#input-res-3").fileinput({
			   ...
			   /* uploadExtraData: {
				...
				initialPreviewDownloadUrl:<?php echo ($downurl); ?>, // includes the dynamic `filename` tag to be replaced for each config
				//initialPreviewDownloadUrl: 'http://10.120.1.243/webtools/PCN/Uploads/{filename}',
			   // overwriteInitial: false,
				 /*  ！这里是预览的数据格式
										initialPreview: [
											// IMAGE RAW MARKUP

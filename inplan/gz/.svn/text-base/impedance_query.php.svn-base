	...
	$imp_query = "select i.item_name
					...
					,(select cop_l.layer_index from items icop,copper_layer cop_l 
					 ...
					 ,nvl((SELECT double_value
						...
						and ICP.CONSTRAINT_SEQUENTIAL_INDEX = IC.SEQUENTIAL_INDEX and ICP.NAME='xcord2' ),0) as pith
					 ,(SELECT double_value
						FROM impedance_constraint_parameter icp where ICP.ITEM_ID=ic.item_id and icp.revision_id=ic.revision_id 
						and ICP.CONSTRAINT_SEQUENTIAL_INDEX = IC.SEQUENTIAL_INDEX and ICP.NAME='sig_gnd_spacing' ) as space
				from items i
					,items imp

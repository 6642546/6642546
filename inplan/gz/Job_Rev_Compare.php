...
echo "<center>Only compare the same layer number </center>";
$first_job=strtoupper($_GET['first_job']);
$second_job=strtoupper($_GET['second_job']);
echo "<div class='form'>
...
$row = oci_fetch_array($job_info, OCI_RETURN_NULLS);
 ...
 $table_p="<table id='top_tab' class='tab' border='0' >
...
$table_p.="<tr><td  align=center style='font-size:160%;font-style:bold;'> Parameter</td><td align=center style='font-size:160%;font-style:bold;'); >Parameter</td></tr>";
...
//$job_p_b=Para('A062517H-301',$conn);
//A062517H-302
$job_p_a=Para($first_job,$conn);  
$job_p_b=Para($second_job,$conn);

	   $rka=array_keys($job_p_a);
	   $rkb=array_keys($job_p_b);
	  
       if (count($rka)>=count($rkb)){
	       foreach($rka as $rkeya=>$rvaluea){
                $ip=0;
				foreach($rkb as $rkeyb=>$rvalueb){
				     if($rvaluea==$rvalueb){
					   $rkc[$rvaluea]=$rvalueb;
					   $ip=1;
					 }
           ...
           foreach(array_diff($rkb,$rka) as $rkd=>$rkval){
		          $rkc['']=$rkval;
		   }
		   ...
		   foreach(array_diff($rka,$rkb) as $rkd=>$rkval){
				  $rkc[$rkval]='';
		   }
	   }  
//print_r(($rkc));
...
foreach($rkc as $key=>$value){
	  $table_p.="<tr><td align=center style='font-size:130%;font-style:bold;'>$key</td><td align=center style='font-size:130%;font-style:bold;')>$value</td></tr>";
	   //$ka=array_keys($value);
	   $ka=array_keys($job_p_a[$key]);
	   $kb=array_keys($job_p_b[$value]);
       if (count($ka)>=count($kb)){
		   ...
		   $rip=0;
           foreach(array_diff($kb,$ka) as $kd=>$kval){
		          $kc[$rip]=$kval;
				  $rip=$rip+1;
		   }
		   ...
		   foreach(array_diff($ka,$kb) as $kd=>$kval){
				  $kc[$kval]='';
		   }
	   }  
     // echo "<br>";
	 ...
	 // echo "<br>";
	  ...
	  foreach($kc as $key1=>$valuep){
          ...
          //Section Note
            $notea=$job_p_a[$key][$key1][0];
			$noteb=$job_p_b[$value][$valuep][0];
            foreach(array_intersect($notea,$noteb) as $nkey=>$nval){
			    if ($nval!=''){
			        $table_p.="<tr><td ><div style=' margin-left:40px;'>$nval</div></td><td><div style=' margin-left:40px;'>$nval</div></td></tr>";
				}
			}
	       foreach(array_diff($notea,$noteb) as $nnkey=>$nnvalue){
				if ($nnvalue!=''){
				 $table_p.="<tr><td><div style=' margin-left:40px;background:#FFFF33;'>$nnvalue</div></td><td><div style='margin-left:40px;background:#FFFF33;'></div></td></tr>";
				}
            }
		   foreach(array_diff($noteb,$notea) as $nnnkey=>$nnnvalue){
				if ($nnnvalue!=""){
				  $table_p.="<tr><td><div style=' margin-left:40px;background:#FFFF33;'></div></td><td><div style='margin-left:40px;background:#FFFF33;'>$nnnvalue</div></td></tr>";
                }
            }
			if (count($notea)==0&&count($noteb)!=0){
			    foreach($noteb as $notekey => $notevalue){
				if ($notevalue!=""){
				  $table_p.="<tr><td><div style=' margin-left:40px;background:#FFFF33;'></div></td><td><div style='margin-left:40px;background:#FFFF33;'>$notevalue</div></td></tr>";
                }
				}
			}
			if (count($noteb)==0&&count($notea)!=0){
			 foreach($notea as $notekey => $notevalue){
				if ($notevalue!=""){
				  $table_p.="<tr><td><div style=' margin-left:40px;background:#FFFF33;'>$notevalue</div></td><td><div style='margin-left:40px;background:#FFFF33;'></div></td></tr>";
                }
			 }
              }
			unset($notea);
		 ...
		 //Section Attr
		  ...
		  if (is_array($value1[1])&&is_array($job_p_b[$value][$valuep][1])){			
              if (count(array_diff($value1[1],$job_p_b[$value][$valuep][1]))==0){
				  ...
				  $out1 = array_diff($value1[1], $job_p_b[$value][$valuep][1]);
                  $out2 = array_diff($job_p_b[$value][$valuep][1], $value1[1]);
				  ...
				  $result=array_intersect($value1[1], $job_p_b[$value][$valuep][1]);
                ...
                unset($out3);
			     ...
			     //  echo $value1;	  
                         if (!is_array($value1[1])&&is_array($job_p_b[$value][$valuep][1])){  
							 foreach($job_p_b[$value][$valuep][1] as $keyk=>$valk){
							   if ($valk!=''){
...
foreach($job_process as $key=>$value){
	    //  echo $key.'<br>';
         ...
         while ($row = oci_fetch_array($rs_trav, OCI_RETURN_NULLS)){
                ...
                //Section Note
				$rs_attr_r1 ="select note_string from  note_trav_sect where item_id=".$row["ITEM_ID"] ." and revision_id=  ".$row["REVISION_ID"]." and SECTION_SEQUENTIAL_INDEX=".$row["SEQUENTIAL_INDEX"]." order by SECTION_SEQUENTIAL_INDEX ASC ";
		        $rs_attr1 = oci_parse($conn, $rs_attr_r1);
                oci_execute($rs_attr1, OCI_DEFAULT);
				$p_para='';
                while ($row_a1 = oci_fetch_array($rs_attr1, OCI_RETURN_NULLS)){
				     $p_para.=trim(str_replace( " ", "",strtoupper($row_a1['NOTE_STRING']))).'#';		
				}
			   $sub_p[$row['OPERATION_CODE'].'      '.$row['DESCRIPTION']][0]=explode('#',$p_para);
               //section attr
			   ...
			   $sub_p[$row['OPERATION_CODE'].'      '.$row['DESCRIPTION']][1]=explode('#',$p_para);
		  }
		  $job_para[$key]=$sub_p;
		  unset($sub_p);
}

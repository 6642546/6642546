...
<?php 
	...
	if ($subparts and substr($process,0,3) !="750" and substr($process,0,3) !="950" ) $mat_table  .=$subparts;
	if (substr($process,0,3) =="950") {
				$mat_list = getCoinBom($process);
				$mat_table  .= $mat_list;
			}
	$mat_table  .= "</table>";
	...
	while(oci_fetch($rstrav)){
		if(oci_result($rstrav, 2)!==$ordernum){
			...
			if (substr($process,0,3) =="950" and oci_result($rstrav, 2)<>"Coin Assembly") {
				$tmpInfo = str_replace($mat_list,"",$tmpInfo);
			}
	...
	function getCoinBom($job){
		//connect to oracle prod
		$sid='edb_prod.merix.com:1601/prod';
		$dbusername='MERIX_INTERFACE';
		$dbpassword='bear514';
		$conn = oci_connect($dbusername, $dbpassword, $sid,'utf8'); 
		
		$result="";
		$query = "SELECT PARENT.segment1,mp.organization_code org
					,PARENT.inventory_item_id
					,CHILD.segment1 child_item
					,CHILD.description child_description
					,CHILD.primary_uom_code,bc.component_quantity 
					,bbom.alternate_bom_designator
					,mir2.revision_label
			FROM apps.mtl_system_items_b PARENT
					,apps.mtl_parameters mp
					,apps.bom_structures_b bs
					,apps.bom_components_b bc
					,apps.mtl_system_items_b CHILD
					,BOM_BILL_OF_MATERIALS bbom
					,mtl_item_revisions_b mir2
			WHERE PARENT.inventory_item_id = bs.assembly_item_id
					AND PARENT.organization_id = bs.organization_id 
					AND PARENT.organization_id = mp.organization_id
					AND bs.bill_sequence_id = bc.bill_sequence_id 
					AND bc.component_item_id = CHILD.inventory_item_id
					AND PARENT.organization_id = CHILD.organization_id 
					AND CHILD.segment1<>'0368310002-C'
					AND bbom.assembly_item_id = PARENT.inventory_item_id
					AND bbom.organization_id= PARENT.organization_id
					AND bbom.alternate_bom_designator is null
					AND bbom.bill_sequence_id=bs.bill_sequence_id
					AND mp.organization_code in ('HY','HZ')
					AND PARENT.segment1 = '$job' 
					AND mir2.inventory_item_id = PARENT.inventory_item_id
					AND mir2.organization_id = PARENT.organization_id
					AND to_number(nvl(mir2.revision_label,0))=
					to_number((SELECT max(NVL (mir.revision, 0))
							FROM mtl_item_revisions mir
							,mtl_system_items_b msi1
							,mtl_parameters mp1
							WHERE msi1.inventory_item_id = mir.inventory_item_id(+)
							AND msi1.organization_id = mir.organization_id(+)
							AND msi1.organization_id = mp1.organization_id
							AND mp1.organization_code in ('HY','HZ')
							AND msi1.segment1 = PARENT.segment1))
					AND (SELECT effectivity_date FROM mtl_item_revisions_b mir3
					WHERE(mir3.inventory_item_id = PARENT.inventory_item_id) AND rownum=1 and mir3.organization_id = PARENT.organization_id
							AND mir3.revision_label= mir2.revision_label) BETWEEN bc.effectivity_date AND NVL (bc.disable_date, SYSDATE + 1)
							ORDER BY CHILD_ITEM";
		
		$stid = oci_parse($conn, $query);
		$r = oci_execute($stid, OCI_DEFAULT);
		while($row = oci_fetch_array($stid, OCI_RETURN_NULLS)) {
			$result .= "<tr><td>"."Subpart"."</td><td>".$row['CHILD_ITEM']."</td><td></td><td>".$row['CHILD_DESCRIPTION']."</td></tr>";
		}
		return $result;
	}

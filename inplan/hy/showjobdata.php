...
$query = "select i.item_name,
				 ...
				 part_da.BP_REV_,
				 (select ev.value from enum_types et,enum_values ev where et.enum_type=ev.enum_type 
						and et.type_name='UL_CODES_' and ev.enum=part_da.UL_CODE_) as UL_CODE_,
			     (select ev.value from enum_types et,enum_values ev where et.enum_type=ev.enum_type 
                 ...
                 (select ev.value from enum_types et,enum_values ev where et.enum_type=ev.enum_type 
						and et.type_name='THICKNESS_CALC_METHOD' and ev.enum=job_da.REQ_THICKNESS_CALC_METHOD_) as THICKNESS_CALC_METHOD,
				 ...
				 ,job_da.read_me_
				 ,(select min(check_in_out_date) from revisions where item_id=i.item_id) as CREATION_DATE
...
echo "<tr class='inplan_attri_tr'><td>".getLang('Order Status',$lang)."</td><td class='inplan_attri_td'>".$row['ORDER_STATUS_']."</td><td>".getLang('Mfg Class',$lang)."</td><td class='inplan_attri_td'>".($row['MFG_CLASS_']!=0?$row['MFG_CLASS_']:'N/A')."</td></tr>";
...
echo "<tr class='inplan_attri_tr'><td>".getLang('Job Creation Date',$lang)."</td><td class='inplan_attri_td'>".gmdate('Y-m-d', strtotime($row['CREATION_DATE']) + 3600 * 8)."</td><td>".getLang('Job Approval Date',$lang)."</td><td class='inplan_attri_td'>".gmdate('Y-m-d', strtotime($row['JOB_APPROVED_DATE_']) + 3600 * 8)."</td></tr>";
echo "<tr class='inplan_attri_tr'><td>".getLang('Read Me',$lang)."</td><td class='inplan_attri_td' colspan=3 style='color:red'>".$row['READ_ME_']."</td></tr>";
...
echo "<tr class='inplan_attri_tr'><td>".getLang('Num PCBs In Array',$lang)."</td><td class='inplan_attri_td'>".$row['NUM_PCBS_IN_ARRAY']."</td><td>".getLang('Num Panels In Sheet',$lang)."</td><td class='inplan_attri_td'>".$row['PANEL_QUANTITY_']."</td></tr>";
...
echo "<tr class='inplan_attri_tr'><td>".getLang('Legend Type',$lang)."</td><td class='inplan_attri_td'>".$row['LCM_INK_TYPES_']."</td><td class='inplan_attri_tr'>".getLang('Legend Color',$lang)."</td><td class='inplan_attri_td'>$silk_color</td></tr>";

if ($row['SOLDERMASK_PEELABLE_SIDES_'] == 'None'){
	echo "<tr class='inplan_attri_tr'><td>".getLang('Peelable Mask Req',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td><td class='inplan_attri_tr'>".getLang('Peelable Mask Application',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td></tr>";
	echo "<tr class='inplan_attri_tr'><td>".getLang('Peelable Mask Material Type',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td><td class='inplan_attri_tr'>".getLang('Peelable Mask Type',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td></tr>";
} else 
{
...
if($row['SELECTIVE_PLATE_'] == "None") {
	echo "<tr class='inplan_attri_tr'><td>".getLang('Selective Plate',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td><td class='inplan_attri_tr'>".getLang('Min Selective Plate Thk',$lang)."</td><td class='inplan_attri_td'>N/A</td></tr>";
} else
{
...
if ($row['PLATED_TIPS_SIDES_'] == "None"){
	echo "<tr class='inplan_attri_tr'><td>".getLang('Plated Tips Required',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td><td class='inplan_attri_tr'>".getLang('Tip Style',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td></tr>";
echo "<tr class='inplan_attri_tr'><td>".getLang('Tip Type',$lang)."</td><td class='inplan_attri_td'>"."N/A"."</td><td class='inplan_attri_tr'>".getLang('Min Gold Tip Thickness',$lang)."</td><td class='inplan_attri_td'>N/A</td></tr>";
} else {
...
$query = "select i.item_name
	...
	while($row = oci_fetch_array($stid, OCI_RETURN_NULLS)) {
		echo "<tr><td>".$row["LAYER"]."</td><td>".($row["LINE_MIN_"]!=0?$row["LINE_MIN_"]:'N/A')."</td><td>".($row["SPACE_MIN_"]!=0?$row["SPACE_MIN_"]:'N/A')."</td><td>".($row["AR_MIN_"]!=0 ? $row["AR_MIN_"] : 'N/A')."</td></tr>";
	}

<?php
	...
	PHPSpellCheck by PHPSpellCheck.com
	
	(C)2010 Jacob Mellor - me@jacobmellor.com - All rights reserved

	This is not open source software - de-obfuscation of this source breaks the license agreement for this software
	
	This code has not been strongly obfuscated for a good reason - to avoid commercial PHP bytcode encoders beacuase they hamper compatibility and add prorietary DLLs to your PHP server
	
	I can see that you are both smart and cusrious, so... please do not distribute this code - nor any derivative works
	
	I am just an independant sofware developer with dyslexia trying to earn an honest crust.
	
	*/
	//  error_reporting(E_ERROR | E_WARNING | E_PARSE | E_NOTICE | E_STRICT | E_ALL |E_RECOVERABLE_ERROR | E_DEPRECATED | E_USER_DEPRECATED );
...
class PHPSpellCheck{
	...
	function Suggestions( $word ){
		...
		$this->Suggestions_CACHE [$word] = $All;
	...
	return $bin_Binary_Loader == $this->_phpName ||$this->_unName ;
	...
	function BinarySearch( $array, $key, $low, $high ){
		...
		elseif( array_key_exists($middle, $array)  && $key < $array  [ $middle ] )// our key might be in the left sub-array
			{
			return PHPSpellCheck::BinarySearch( $array, $key, $low, $middle-1 );
		}

		return PHPSpellCheck :: BinarySearch( $array, $key, $middle+1, $high );

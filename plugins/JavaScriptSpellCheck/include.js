...
if(docs[i]!==oField[t]){if(t==="iframe"){var myFrame=oField;var oDoc=livespell.getIframeDocumentBasic(oField);var oBody=oDoc.body;if(oBody.innerHTML!=docs[i]){oBody.innerHTML=docs[i]};}else{if(livespell.insitu.proxyDOM(id)){oField[t]=docs[i];}else{oField[t]=livespell.str.HTMLDec(docs[i]);}}
...
for(var i=0;i<fieldIds.length;i++){var id=fieldIds[i];var oField=document.getElementById(id);var t=this.FieldType(id);var val;if(t==="iframe"){var oDoc=livespell.getIframeDocumentBasic(oField);val=oDoc.body.innerHTML;}else{val=oField[t];val=livespell.str.HTMLEncSafeWhite(val);val=livespell.str.stripTags(val);}
docs[i]=val;}
return docs;}
...
var fn=function(){f["onDialogOpen"]()};setTimeout(fn,189)}else if(wm=="modal"&&window.showModalDialog&&canmodal){var dheight=livespell.test.IE6()?333:290;window.showModalDialog(this.url(),window,"center:1;dialogheight:"+dheight+"px;dialogwidth:460px;resizable:0;scrollbars:0;scroll:0;location:0");}
else if(wm=="modeless"&&window.showModelessDialog&&canmodal){var dheight=livespell.test.IE6()?333:290;window.showModelessDialog(this.url(),window,"center:1;dialogheight:"+dheight+"px;dialogwidth:460px;resizable:0;scrollbars:0;scroll:0;location:0");}
...
var params='';params+="note="+(note);params+="&command="+(cmd);params+="&args="+(args);params+="&lan="+(lan);params+="&sender="+(sender);params+="&settingsfile="+(settingsfile);if(hasajax){xhr.onreadystatechange=function(){if(xhr.readyState==4){if(xhr.status==200){livespell.ajax.pickup(xhr.responseText,false);}
else{if(livespell.rubberRingServerModel!=livespell.setrubberRingServerModel()){livespell.ajax.resend()}
if(typeof(window['LIVESPELL_DEBUG_MODE'])!="undefined"){try{if(xhr.responseText.length){livespell.ajax.debug("SERVER ERROR:"+xhr.responseText,false)}else{livespell.ajax.debug("SERVER ERROR - Please view this page in a browser other than IE for full details",false)}}catch(e){livespell.ajax.debug("SERVER ERROR",false)}}}}};xhr.open("GET",posturl+"?"+params,false);xhr.send();livespell.ajax.debug("URL:"+posturl+"  <br/>  GET:"+params,true)}else{var e;livespell.ajax.renderIframe(posturl);params+="&script=true"
...
catch(e){theframe.src=posturl+"?"+params}}},send_sync:function(cmd,args,lan,note,sender){livespell.cache.ajaxrequest={};livespell.cache.ajaxrequest.cmd=cmd;livespell.cache.ajaxrequest.args=args;livespell.cache.ajaxrequest.lan=lan;livespell.cache.ajaxrequest.note=note;livespell.cache.ajaxrequest.sender=sender;var oSender=livespell.spellingProviders[sender];var serverModel=oSender.ServerModel.toLowerCase()
...
strinput=strinput.replace((/<!--[\s\S.\n]*?-->/g),"");strinput=strinput.replace((/&lt;!--[\s\S.\n]*?--&gt;/g),"");return strinput;},HTMLEnc:function(s){s=(s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));s=s.replace(/\n/g,"<br />");s=s.replace(/[ ][ ]/gi," &nbsp;");s=s.replace(/[ ][ ]/gi," &nbsp;");return s;},HTMLEncSafeWhite:function(s){s=(s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));s=s.replace(/[ ][ ]/gi," &nbsp;");s=s.replace(/[ ][ ]/gi," &nbsp;");return s;},HTMLDec:function(s){s=s=s.replace(/\&nbsp\;/gi," ");s=(s.replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>'));return s;},HTMLDecAndStripFormatting:function(s){if(livespell.test.IE()){s=s.replace((/\n/gi),"");}else{s=s.replace((/\n/gi),"<BR>");s=s.replace((/\r/gi),"<BR>");}
...
try{if(t.getAttribute('maxlength')){n.maxLength=t.getAttribute('maxlength');}
...
n.onpaste=function(){try{var value=window.clipboardData.getData("Text",value);window.clipboardData.setData("Text",value);}catch(e){}
...
var ch8r=event.keyCode;var hasRange=false;var txt='';try{if(window.getSelection)
{txt=window.getSelection();}
else if(document.getSelection)
{txt=document.getSelection();}
else if(document.selection)
{txt=document.selection.createRange().text;}}catch(e){}
...
this.keysTrapped=true;}},menukey:function(){if(livespell.context.isOpen()){if(event.preventDefault){event.preventDefault();}
else{event.returnValue=false;}
return false;}
